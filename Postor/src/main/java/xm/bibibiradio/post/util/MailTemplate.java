...
public class MailTemplate {
    ...
    public static String getAlertContent(String category, int level, Date time, String srcIp,
                                         String appName, String dstIp, long rowNum,
                                         long visitCount, String userName, String detail) {
        return String.format(templates.get("getAlertContent"), category, Integer.toString(level),
            dateFormat.format(time), srcIp, appName, dstIp, Long.toString(rowNum),
            Long.toString(visitCount), userName, detail);
    }

    public static String getResourcesContent(List<PostResourceData> postDatas) {
        String head = "<table>" + "<tr>" + "<th>缩略图</th>" + "<th>标题</th>" + "<th>作者</th>"
                      + "<th>点击量</th>" + "<th>评论量</th>" + "<th>分数</th>" + "<th>创建时间</th>" + "</tr>";
        ...
        for(PostResourceData postResourceData:postDatas){
            sb.append(String.format(loopDodyTemplate
                ...
                ,postResourceData.getAuthorName()
                ,postResourceData.getPv()
